...
export default function Header() {
  ...
  const dot =
    status === "running" ? "bg-accent animate-pulseline" : status === "done" ? "bg-pass" : "bg-slate-600";
  const statusText =
    status === "idle" ? "Ready" : status === "done" ? "Complete" : status === "paused" ? "Paused" : "Running";

  return (
    <header className="glass sticky top-0 z-10 flex items-center justify-between border-b hairline bg-black/25 px-6 py-3">
